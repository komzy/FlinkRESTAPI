...
def getTaskManagers(base_url):
    url = base_url + "/taskmanagers"
    mydata = '{}'
    x = requests.get(url, params=mydata)
    task_managers = set()
    total_taskmanagers = x.json()['taskmanagers']
    for tm in total_taskmanagers:
        task_managers.add(tm["id"])
    return task_managers

def getTaskManagerMetrics(base_url, task_managers):
    metrics = {}
    for tm in task_managers:
        url = base_url + "/taskmanagers/" + tm + "/metrics"
